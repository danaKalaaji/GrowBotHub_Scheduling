...
def make_flow_vars():
    ...
    for u, v, d in gc.g.edges(data=True):
        for key, val in d['bounds'].items():
            if(val > 0):
                if key[0].name == "Strawberry":
                    if not (u.type == "source" and v.when % 10 != 0) :
        ...
        if v.type == 'module':
            ...
            model += plp.lpSum(bundle) <= inputs.HOLES[int(v.where/2)]
...
def balance_constraint():
    ...
    limit = (plp.lpSum(sink_inflow) / (edges.n_plants-1)) - alpha
    sink = [[] for _ in range(edges.n_plants - 1)]

    i = 0
    for n in [n for n in gc.g.nodes if n.sink and n != edges.meta_nodes[index_strawberry()][1]]:
        for e in gc.g.in_edges(n):
            for c in plants.plants:
                if c[0].name != "Strawberry":
                    if (e[0], e[1], c) in flow_vars:
                        sink[i] += [flow_vars[e[0], e[1], c]]
        model += plp.lpSum(sink[i]) >= limit
        i = i+1


def index_strawberry():
    for i in range(edges.n_plants):
            if(inputs.plant_data[i].name == "Strawberry"):
                return i 
...
def strawberries():
    strawberries = []
    for n in (n for n in gc.g.nodes if n.type == 'source'):
        for e in (e for e in gc.g.out_edges(n)): #if e[0].type == 'hole'):
            for c in plants.plants:
                if c[0].name == "Strawberry":
                    if (e[0], e[1], c) in flow_vars:
                        strawberries += [flow_vars[e[0], e[1], c]]
    print("strawberries array has lenght", len(strawberries))
    return strawberries
...
def optimize():
    global model
    u = 10000000000000000000000
    w = 7
    model += plp.lpSum(get_sink_inflow()) + u* plp.lpSum(strawberries()) - w* plp.lpSum(z()) #- v * plp.lpSum(y())
