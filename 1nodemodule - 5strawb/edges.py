...
An edge is defined by:
	...
	bounds (plants, value): with value = [0,1]
...
## GROWTH MODULE EDGES ##
'''
Creates all the edges that stay in the same growth module from a day to another.
For each module and for each day, createss an edge that connects this module to the 
same module on the next day and store it in the array tray_edges
'''
def make_tray_edges():
	number_tray_edges = 0
	global tray_edges
	...
	for module_type in range(inputs.NB_TYPE_MODULE):
		for module_number in range(inputs.MODULES[module_type]):
			for t in range(inputs.HORIZON):
				node_from = classes.Node('module', module_type, module_number, t)
				node_to = classes.Node('module', module_type, module_number, t + 1)
				edge = Edge(node_from, node_to, get_bounds_per_edge(module_type, t, plants_array.plants), get_sizes_per_edge(module_type, t, plants_array.plants))
...
def make_transfer_edges(): 
	...
	for plant in plants_array.plants:
		...
		for i in range(len(plant_type.transfers) - 1):
			...
			for module_number in range(inputs.MODULES[from_]):
				node_from = classes.Node('module', from_, module_number, t)

				for module_number_to in range(inputs.MODULES[to_]):
					node_to = classes.Node('module', to_, module_number_to, t+1)
...
'''
def is_module_type_in_plant(module_type, plant):
    return module_type in plant.transfers
...
Parameters:
	...
	plant (Plant): the plant we are considering
'''
def get_module_type_index(module_type, plant):
    return plant.transfers.index(module_type)


'''
Computes all the bounds for each type of plant at a given time and in a
given growth module. Bounds are 0 or 1:
...
'''
def get_bounds_per_edge(module_type, t, plants):
    bounds_dict = dict()

    for plant in plants:
        day = plant[1]
        this_plant = plant[0]
        if is_module_type_in_plant(module_type, this_plant):
            day_from = day + \
                this_plant.transfer_days[get_module_type_index(module_type, this_plant)][0]
            day_to = day + \
                this_plant.transfer_days[get_module_type_index(
                    module_type, this_plant)][1] - 1
...
'''
def get_sizes_per_edge(module_type, t, plants):
    sizes_dict = dict()
    ...
    for plant in plants:
        ...
        if is_module_type_in_plant(module_type, plant_type):
            day_from = day + \
                plant_type.transfer_days[get_module_type_index(module_type, plant_type)][0]
            day_to = day + \
                plant_type.transfer_days[get_module_type_index(
                    module_type, plant_type)][1] - 1
