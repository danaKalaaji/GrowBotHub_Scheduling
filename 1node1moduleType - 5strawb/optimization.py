...
Each edge connecting a node of type A has a
    -total capacity of 5*(number of modules of type A)
    -capacity per commodity of 2*(number of modules of type A)
'''
def make_flow_vars():
    ...
    for u, v, d in gc.g.edges(data=True):
        ...
        if v.type == 'modules':
            ...
            model += plp.lpSum(bundle) <= inputs.HOLES[int(v.modules_type)]
...
def size_constraint():
    ...
    for max_size in inputs.MAX_SIZE.keys():
        for pair in (pair for pair in pair_of_neighbors(gc.g) if (pair[0].hole, pair[1].hole) in inputs.MAX_SIZE[max_size]):
            bundle = []
            for e in (e for e in gc.g.in_edges(pair[0], data=True) if e[0].type == 'modules'):
                for c in e[2]['size'].keys():
                    if (e[0], pair[0], c) in flow_vars:
                        bundle += [flow_vars[e[0], pair[0], c]
                                   * e[2]['size'].get(c)]
            for e in (e for e in gc.g.in_edges(pair[1], data=True) if e[0].type == 'modules'):
                for c in e[2]['size'].keys():
...
'''
Adds the size constraint to the model.
There cannot be more than 2*(number of modules of type 2) big plants (Lettuce, Cabbage, Fennel) in each module of type 2.
'''
def size_constraint():
    global model
                ...
                for c in plants.plants:
                    if (e[0], e[1], c) in flow_vars:
                        if c[0].name == "Lettuce":
                            ...
                            lettuce_cabbage_module += [flow_vars[e[0], e[1], c]]
                            fennel_cabbage_module += [flow_vars[e[0], e[1], c]]
        model += plp.lpSum(lettuce_fennel_module) <= 2*inputs.MODULES[int(v.modules_type)]
        model += plp.lpSum(lettuce_cabbage_module) <= 2*inputs.MODULES[int(v.modules_type)]
        model += plp.lpSum(fennel_cabbage_module) <= 2*inputs.MODULES[int(v.modules_type)]
        lettuce_fennel_module = []
        lettuce_cabbage_module = []
        fennel_cabbage_module = []
...
The inflow for all nodes in the graph (except sources ans sinks) should not exceed the number of holes per growth modules type
...
def max_inflow_constraint():
    ...
    for n in (n for n in gc.g.nodes if n.type == 'modules'):
        for e in (e for e in gc.g.in_edges(n)): #if e[0].type == 'hole'):
            for c in plants.plants:
                if (e[0], e[1], c) in flow_vars:
                    inflow_from_holes[list(gc.g.nodes).index(n)] += [flow_vars[e[0], e[1], c]]
                    #print(e[0].modules_type, e[0].hole, e[0].when , "||",e[1].modules_type, e[1].hole, e[1].when, "||",flow_vars[e[0], e[1], c].varValue)

        model += plp.lpSum(inflow_from_holes[list(gc.g.nodes).index(n)]) <= inputs.HOLES[int(n.modules_type)]
...
def optimize():
    ...
    # Solves without timeout or a cut
    #model.solve()
...
def pair_of_neighbors(g):
    pairs = set()
    for n1 in (n1 for n1 in gc.g.nodes if n1.type == 'modules' and n1.where % 2 == 0):
...
#constraint: at most x plants of type p every 3 days 
#--> weight if we harvest plants of type p more than once every 3 days
"""
def y():
    global model
    y = dict()
