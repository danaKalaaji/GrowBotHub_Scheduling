...
def make_flow_vars(TRAYS_PER_TYPE, HOLES, model):
    ...
    for u, v, d in gc.g.edges(data=True):
        for key, val in d['bounds'].items():
            if(val > 0):
                ...
                if v.type == 'module':
                    ...
                    model += flow_vars[u, v, key] <= 2 * TRAYS_PER_TYPE[0]
        ...
        if v.type == 'module':
            ...
            model += plp.lpSum(bundle) <= HOLES[int(v.where/2)]
...
def max_inflow_constraint(HOLES, model):
    ...
    for n in (n for n in gc.g.nodes if n.type == 'module'):
        ...
        model += plp.lpSum(inflow_from_holes[list(gc.g.nodes).index(n)]) <= HOLES[int(n.where/2)]
...
def flow_conservation_constraint(model):
    for n in (n for n in gc.g.nodes if n.type == 'module'):
        for c in plants.plants:
...
def optimize(model):
    w = 7
    model += plp.lpSum(get_sink_inflow()) - w* plp.lpSum(z(model)) #- v * plp.lpSum(y())
